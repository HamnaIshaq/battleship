...
describe("Game tests", () => {
  test("game round", () => {
    ...
    expect(player1.makeMoveOnBoard(player2.gameBoard, [0, 5])).toBe(
      "ship was hit!"
    );
    expect(player2.makeMoveOnBoard(player1.gameBoard, [0, 5])).toBe("missed");

    expect(player1.makeMoveOnBoard(player2.gameBoard, [0, 6])).toBe(
      "ship has sunk!"
    );
    expect(player2.makeMoveOnBoard(player1.gameBoard, [0, 0])).toBe(
      "ship was hit!"
    );

    expect(player1.makeMoveOnBoard(player2.gameBoard, [2, 2])).toBe(
      "ship was hit!"
    );
    expect(player2.makeMoveOnBoard(player1.gameBoard, [0, 1])).toBe(
      "ship has sunk!"
    );

    expect(player1.makeMoveOnBoard(player2.gameBoard, [2, 3])).toBe(
      "ship was hit!"
    );
    expect(player2.makeMoveOnBoard(player1.gameBoard, [0, 9])).toBe("missed");
    expect(player1.makeMoveOnBoard(player2.gameBoard, [2, 4])).toBe(
      "ship has sunk!"
    );
    expect(player2.makeMoveOnBoard(player1.gameBoard, [0, 7])).toBe("missed");
